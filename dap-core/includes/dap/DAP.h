...
#if (DAP_CORE_JTAG_DEV_CNT != 0)
    uint8_t   ir_length[DAP_CORE_JTAG_DEV_CNT];      // IR Length in bits
    uint16_t  ir_before[DAP_CORE_JTAG_DEV_CNT];      // Bits before IR
    uint16_t  ir_after [DAP_CORE_JTAG_DEV_CNT];      // Bits after IR
#endif
  } jtag_dev;
#endif
} DAP_Data_t;
