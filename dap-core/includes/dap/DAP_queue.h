...
#define FREE_COUNT_INIT          (DAP_CORE_PACKET_COUNT)
#define SEND_COUNT_INIT          0

typedef struct {
    uint8_t     USB_Request [DAP_CORE_PACKET_COUNT][DAP_CORE_PACKET_SIZE];  // Request  Buffer
    uint16_t    resp_size[DAP_CORE_PACKET_COUNT]; //track the return response size
    uint32_t    free_count;
    uint32_t    send_count;
    uint32_t    recv_idx;
    uint32_t    send_idx;
