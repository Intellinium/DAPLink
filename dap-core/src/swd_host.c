...
static uint8_t swd_read_block(uint32_t address, uint8_t *data, uint32_t size)
{
	...
	for (i = 0; i < (size_in_words - 1); i++) {
		if (swd_transfer_retry(req, (uint32_t *) data)
			!= DAP_TRANSFER_OK) {
			return 0;
		}
...
uint8_t swd_read_memory(uint32_t address, uint8_t *data, uint32_t size)
{
	...
	while (size > 3) {
		...
		n = TARGET_AUTO_INCREMENT_PAGE_SIZE
			- (address & (TARGET_AUTO_INCREMENT_PAGE_SIZE - 1));

		if (size < n) {
			n = size
				& 0xFFFFFFFC; // Only count complete words remaining
		}
...
static uint8_t swd_write_debug_state(DEBUG_STATE *state)
{
	...
	if (!swd_write_word(DBG_HCSR,
			    DBGKEY | C_DEBUGEN | C_MASKINTS | C_HALT)) {
		return 0;
	}
...
uint8_t
swd_flash_syscall_exec(const program_syscall_t *sysCallParam, uint32_t entry,
		       uint32_t arg1, uint32_t arg2, uint32_t arg3,
		       uint32_t arg4, flash_algo_return_t return_type)
{
...
static uint8_t swd_read_idcode(uint32_t *id)
{
	...
	*id = (tmp_out[3] << 24) | (tmp_out[2] << 16) | (tmp_out[1] << 8)
		| tmp_out[0];
	return 1;
}
...
uint8_t swd_set_target_state_sw(target_state_t state)
{
	...
	case RESET_PROGRAM:
		...
		if (!swd_write_word(NVIC_AIRCR,
				    VECTKEY | (val & SCB_AIRCR_PRIGROUP_Msk)
					    | soft_reset)) {
			return 0;
		}
