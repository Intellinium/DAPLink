...
void Delayms(uint32_t delay) {
  delay *= ((system_clock_get()/1000U) + (DELAY_SLOW_CYCLES-1U)) / DELAY_SLOW_CYCLES;
...
static uint32_t DAP_Delay(const uint8_t *request, uint8_t *response) {
  ...
  delay *= ((system_clock_get()/1000000U) + (DELAY_SLOW_CYCLES-1U)) / DELAY_SLOW_CYCLES;
...
static uint32_t DAP_SWJ_Pins(const uint8_t *request, uint8_t *response) {
  ...
  if (wait != 0U) {
    ...
    do {
      if ((select & (1U << DAP_SWJ_SWCLK_TCK)) != 0U) {
        if ((value >> DAP_SWJ_SWCLK_TCK) ^ sw_dp_swdclk_get()) {
          continue;
        }
      }
      if ((select & (1U << DAP_SWJ_SWDIO_TMS)) != 0U) {
        if ((value >> DAP_SWJ_SWDIO_TMS) ^ sw_dp_swdio_get()) {
          continue;
        }
      }
      if ((select & (1U << DAP_SWJ_TDI)) != 0U) {
      ...
      if ((select & (1U << DAP_SWJ_nTRST)) != 0U) {
        if ((value >> DAP_SWJ_nTRST) ^ jtag_trst_get()) {
          continue;
        }
      }
      if ((select & (1U << DAP_SWJ_nRESET)) != 0U) {
        if ((value >> DAP_SWJ_nRESET) ^ target_rdst_get()) {
          continue;
        }
      }
      break;
...
void DAP_Setup(void) {
  ...
  if (DAP_CORE_DEFAULT_SWJ_CLOCK >= MAX_SWJ_CLOCK(DELAY_FAST_CYCLES)) {
    ...
    uint32_t delay = ((system_clock_get()/2U) + (DAP_CORE_DEFAULT_SWJ_CLOCK - 1U)) / DAP_CORE_DEFAULT_SWJ_CLOCK;
    if (delay > DAP_CORE_IO_PORT_WRITE_CYCLES) {
      delay -= DAP_CORE_IO_PORT_WRITE_CYCLES;
      delay  = (delay + (DELAY_SLOW_CYCLES - 1U)) / DELAY_SLOW_CYCLES;
    } else {
