...
#define MAX_SWJ_CLOCK(delay_cycles) \
  ((system_clock_get()/2U) / (DAP_CORE_IO_PORT_WRITE_CYCLES + delay_cycles))

#define CLOCK_DELAY(swj_clock) \
 (((system_clock_get()/2U) / swj_clock) - DAP_CORE_IO_PORT_WRITE_CYCLES)
...
static uint8_t DAP_Info(uint8_t id, uint8_t *info)
{
	...
	case DAP_ID_CAPABILITIES:
		info[0] = (IS_ENABLED(DAP_CORE_SWD_ENABLED) ? (1U << 0) : 0U) |
			(IS_ENABLED(DAP_CORE_JTAG_ENABLED) ? (1U << 1) : 0U) |
			(DAP_CORE_TIMESTAMP_CLOCK != 0U ? (1U << 5) : 0U)
#if 0
			/* FIXME !!
					| ((SWO_UART != 0)         ? (1U << 2) : 0U) |
...
#if (DAP_CORE_TIMESTAMP_CLOCK != 0U)
		info[0] = (uint8_t)(DAP_CORE_TIMESTAMP_CLOCK >> 0);
		info[1] = (uint8_t)(DAP_CORE_TIMESTAMP_CLOCK >> 8);
		info[2] = (uint8_t)(DAP_CORE_TIMESTAMP_CLOCK >> 16);
		info[3] = (uint8_t)(DAP_CORE_TIMESTAMP_CLOCK >> 24);
		length = 4U;
#endif
		break;
	...
	case DAP_ID_PACKET_SIZE:
		info[0] = (uint8_t)(DAP_CORE_PACKET_SIZE >> 0);
		info[1] = (uint8_t)(DAP_CORE_PACKET_SIZE >> 8);
		length = 2U;
		break;
	case DAP_ID_PACKET_COUNT:
		info[0] = DAP_CORE_PACKET_COUNT;
		length = 1U;
		break;
	default:
		break;
...
{
	delay *= ((system_clock_get() / 1000U) + (DELAY_SLOW_CYCLES - 1U))
		/ DELAY_SLOW_CYCLES;
...
static uint32_t DAP_Delay(const uint8_t *request, uint8_t *response)
{
	...
	delay *= ((system_clock_get() / 1000000U) + (DELAY_SLOW_CYCLES - 1U))
		/ DELAY_SLOW_CYCLES;
...
static uint32_t DAP_HostStatus(const uint8_t *request, uint8_t *response)
{
	...
	case DAP_DEBUGGER_CONNECTED:
		/* TODO LED_CONNECTED_OUT((*(request+1) & 1U));*/
		break;
	case DAP_TARGET_RUNNING:
		/* TODO LED_RUNNING_OUT((*(request+1) & 1U)); */
		break;
	default:
		*response = DAP_ERROR;
		return ((2U << 16) | 1U);
...
static uint32_t DAP_ResetTarget(uint8_t *response)
{
	...
	*(response + 1) = IS_ENABLED(DAP_CORE_TARGET_RESET_ENABLED);
	*(response + 0) = DAP_OK;
	return (2U);
}
...
static uint32_t DAP_SWJ_Pins(const uint8_t *request, uint8_t *response)
{
	...
	if ((select & (1U << DAP_SWJ_TDI)) != 0U) {
		jtag_tdi_set(value >> DAP_SWJ_TDI);
	}
	if ((select & (1U << DAP_SWJ_nTRST)) != 0U) {
		jtag_trst_set(value >> DAP_SWJ_nTRST);
	}
	if ((select & (1U << DAP_SWJ_nRESET)) != 0U) {
		target_rst_set(value >> DAP_SWJ_nRESET);
	}
	...
	if (wait != 0U) {
...
#if (DAP_CORE_TIMESTAMP_CLOCK >= 1000000U)
		wait *= DAP_CORE_TIMESTAMP_CLOCK / 1000000U;
#else
		wait /= 1000000U / DAP_CORE_TIMESTAMP_CLOCK;
#endif
#else
		wait  = 1U;
#endif
		...
		do {
			...
			if ((select & (1U << DAP_SWJ_TDI)) != 0U) {
				if ((value >> DAP_SWJ_TDI) ^ jtag_tdi_get()) {
					continue;
				}
			}
			if ((select & (1U << DAP_SWJ_nTRST)) != 0U) {
				if ((value >> DAP_SWJ_nTRST)
					^ jtag_trst_get()) {
					continue;
				}
			}
	...
	value = (sw_dp_swdclk_get() << DAP_SWJ_SWCLK_TCK) |
		(sw_dp_swdio_get() << DAP_SWJ_SWDIO_TMS) |
		(jtag_tdi_get() << DAP_SWJ_TDI) |
		(jtag_tdo_get() << DAP_SWJ_TDO) |
		(jtag_trst_get() << DAP_SWJ_nTRST) |
		(target_rdst_get() << DAP_SWJ_nRESET);
...
static uint32_t DAP_SWJ_Clock(const uint8_t *request, uint8_t *response)
{
	...
	if (clock >= MAX_SWJ_CLOCK(DELAY_FAST_CYCLES)) {
		...
		delay = ((system_clock_get() / 2U) + (clock - 1U)) / clock;
		if (delay > DAP_CORE_IO_PORT_WRITE_CYCLES) {
			delay -= DAP_CORE_IO_PORT_WRITE_CYCLES;
			delay = (delay + (DELAY_SLOW_CYCLES - 1U))
				/ DELAY_SLOW_CYCLES;
		} else {
			delay = 1U;
		}
	...
	for (; request_count != 0U; request_count--) {
		...
		if ((request_value & DAP_TRANSFER_RnW) != 0U) {
			...
			if ((request_value & DAP_TRANSFER_MATCH_VALUE) != 0U) {
				...
				while (((data & DAP_Data.transfer.match_mask)
					!= match_value) && match_retry--
					&& !DAP_TransferAbort);
				if ((data & DAP_Data.transfer.match_mask)
					!= match_value) {
...
#if (DAP_CORE_TIMESTAMP_CLOCK != 0U)
				// Store Timestamp
				if ((request_value & DAP_TRANSFER_TIMESTAMP)
					!= 0U) {
					timestamp = DAP_Data.timestamp;
	...
	if (response_value == DAP_TRANSFER_OK) {
		if (post_read) {
			...
			*response++ = (uint8_t)(data >> 24);
			...
			do {
				response_value = SWD_Transfer(
					DP_RDBUFF | DAP_TRANSFER_RnW, NULL);
			}
			while ((response_value == DAP_TRANSFER_WAIT) && retry--
				&& !DAP_TransferAbort);
		}
...
static uint32_t DAP_JTAG_Transfer(const uint8_t *request, uint8_t *response)
{
	...
	for (; request_count != 0U; request_count--) {
		request_value = *request++;
		request_ir = (request_value & DAP_TRANSFER_APnDP) ? JTAG_APACC
								  : JTAG_DPACC;
		if ((request_value & DAP_TRANSFER_RnW) != 0U) {
			// Read register
			if (post_read) {
				// Read was posted before
				retry = DAP_Data.transfer.retry_count;
				if ((ir == request_ir) && ((request_value
...
static uint32_t DAP_SWD_TransferBlock(const uint8_t *request, uint8_t *response)
{
	...
	if ((request_value & DAP_TRANSFER_RnW) != 0U) {
		...
		if ((request_value & DAP_TRANSFER_APnDP) != 0U) {
			...
			do {
				response_value =
					SWD_Transfer(request_value, NULL);
			}
			while ((response_value == DAP_TRANSFER_WAIT) && retry--
				&& !DAP_TransferAbort);
			if (response_value != DAP_TRANSFER_OK) {
		...
		while (request_count--) {
			// Read DP/AP register
			if ((request_count == 0U)
				&& ((request_value & DAP_TRANSFER_APnDP)
					!= 0U)) {
				// Last AP read
		...
		do {
			response_value =
				SWD_Transfer(DP_RDBUFF | DAP_TRANSFER_RnW,
					     NULL);
		}
		while ((response_value == DAP_TRANSFER_WAIT) && retry--
			&& !DAP_TransferAbort);
...
#if defined(DAP_CORE_JTAG_ENABLED)
static uint32_t
DAP_JTAG_TransferBlock(const uint8_t *request, uint8_t *response)
{
	uint32_t request_count;
	uint32_t request_value;
	uint32_t response_count;
	...
	if ((request_value & DAP_TRANSFER_RnW) != 0U) {
		...
		while (request_count--) {
			...
			do {
				response_value =
					JTAG_Transfer(request_value, &data);
			}
			while ((response_value == DAP_TRANSFER_WAIT) && retry--
				&& !DAP_TransferAbort);
			if (response_value != DAP_TRANSFER_OK) {
		...
		do {
			response_value =
				JTAG_Transfer(DP_RDBUFF | DAP_TRANSFER_RnW,
					      NULL);
		}
		while ((response_value == DAP_TRANSFER_WAIT) && retry--
			&& !DAP_TransferAbort);
...
//             number of bytes in request (upper 16 bits)
__WEAK uint32_t
DAP_ProcessVendorCommand(const uint8_t *request, uint8_t *response)
{
	(void) request;
...
//             number of bytes in request (upper 16 bits)
__WEAK uint32_t
DAP_ProcessVendorCommandEx(const uint8_t *request, uint8_t *response)
{
	*response = ID_DAP_Invalid;
	return ((1U << 16) | 1U);
}
...
void DAP_Setup(void)
{
	...
	if (DAP_CORE_DEFAULT_SWJ_CLOCK >= MAX_SWJ_CLOCK(DELAY_FAST_CYCLES)) {
		DAP_Data.fast_clock = 1U;
		DAP_Data.clock_delay = 1U;
	} else {
		DAP_Data.fast_clock = 0U;

		uint32_t delay = ((system_clock_get() / 2U)
			+ (DAP_CORE_DEFAULT_SWJ_CLOCK - 1U))
			/ DAP_CORE_DEFAULT_SWJ_CLOCK;
		if (delay > DAP_CORE_IO_PORT_WRITE_CYCLES) {
			delay -= DAP_CORE_IO_PORT_WRITE_CYCLES;
			delay = (delay + (DELAY_SLOW_CYCLES - 1U))
				/ DELAY_SLOW_CYCLES;
